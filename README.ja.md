...
```
">=3.5.0,<4.0.0"   → そのまま維持、4.0.0未満のみ更新
">=1.0,<=2.0"      → そのまま維持、2.0を含めて更新
"4.0.0..<5.0.0"    → そのまま維持、5.0.0未満のみ更新
"4.0.0...4.9.9"    → そのまま維持、4.9.9を含めて更新
"1.2.0 - 2.0.0"    → そのまま維持、2.0.0を含めて更新（npmハイフン範囲）
"[1.0,2.0)"        → そのまま維持、2.0未満のみ更新（Mavenスタイル）
"[1.0,2.0]"        → そのまま維持、2.0を含めて更新（Mavenスタイル）
"[1.0,2.0.Final)"  → そのまま維持、2.0.Final未満のみ更新（Maven qualifier）
"]1.0,2.0["        → そのまま維持、2.0未満のみ更新（Maven代替ブラケット）
"]1.0,2.0]"        → そのまま維持、2.0を含めて更新（Maven代替包含）
"[1.0,2.0["        → そのまま維持、2.0未満のみ更新（Maven上限排他ブラケット）
```

依存関係に上限制約（例：`<4.0.0`、`<=2.0`、`...4.9.9`）がある場合、depupは：
- 上限を超えるバージョンを**提案しません**
