...
depup は、固定されていない依存について公開済みのバージョンを調べ、次の条件を満たす最新のものを選んでマニフェストを書き換えます。デフォルトの動作は次のとおりです。
...
- **対象外**: 更新できる依存として扱わず、出力にも現れないもの。レジストリ以外を指す宣言（Cargo の `path` 依存など）、プラットフォームパッケージ（Composer の `php` など）、浮動指定や未対応の書き方のバージョン（`"*"`、`latest` など）がこれにあたります。

age フィルターや脆弱性チェックが取り除くのは候補のバージョンで、依存そのものではありません。残った候補の中で最新のものへ更新し、現在のバージョンより新しい候補が 1 つも残らなかったときだけスキップとして報告します。

## 更新候補の絞り込み

設定で変えられる 3 つのフィルターが、選べるバージョンを絞り込みます。デフォルトで有効な age フィルターと脆弱性チェック、それに指定したときだけ効く `--max-change` です。どれも実行ごとにオプションで指定するか、[グローバル設定ファイル](#グローバル設定ファイル)でデフォルトを変えられます。プレリリースや、範囲の上限を超えるバージョンも候補から外れます（[バージョン指定と書き換え](#バージョン指定と書き換え)）。
...
プロジェクトに書かれた最小公開期間（pnpm・Bun の `minimumReleaseAge`、mise の `minimum_release_age`）は**プロジェクトポリシー**として扱い、CLI や設定ファイルの値より優先します。実際に適用される age は、次の順で決まります（上ほど優先）。

1. **プロジェクトポリシー**（pnpm・Bun・mise の設定。読み取る設定ファイルは下記）
...
- Swift パッケージは照会しません（OSV は Swift パッケージをリポジトリの完全な URL で識別しますが、depup は GitHub の `owner/repo` で識別するため、照会しても一致しません）。
...
更新先に選んだバージョンに既知の脆弱性が見つかった場合、depup はそれを候補から外し、次に新しい候補を調べます。安全なバージョンが見つかるか、新しい候補がなくなるまで繰り返します。OSV チェックを通過した更新には `✓ OSV` マークが付きます。
...
この例では、3.4.8 を候補から外したあとに条件を満たす dompurify の候補が残らなかったため、dompurify は更新されず、41 件のスキップに含まれています。安全なバージョンが見つかった場合は、更新行の下に `↳ OSV skipped: <バージョン> (<勧告 ID>)` 行が付きます。

`falling back` は、脆弱なバージョンを設計どおりに避けたことを知らせる通知で、終了コードには影響しません。レポートの上にある `⚠ OSV:` 行は標準エラー出力への進捗表示で、端末に出力しているときだけ表示されます（`--quiet` のときは常に表示）。デフォルトのテキスト出力では同じ内容が `Errors:` 欄（JSON では `errors`）にも入るため、CI でも失われません。
...
depup は初回実行時に、デフォルトの設定を説明コメント付きで書いた `~/.config/depup/config.toml` を作成します。既存のファイルは上書きしません。値を編集すると、すべてのプロジェクトに共通するデフォルトを変えられます。1 回の実行だけ変えたい場合はコマンドラインのオプションが優先されます（各節の優先順位を参照）。次の例は生成される雛形の内容です（実際のファイルのコメントは英語です）。
...
ファイルにないキーは、組み込みデフォルト（`age = "1w"`、`osv = true`、`max_change` の制限なし）を使います。ファイルを作成・解析できない場合は、警告を表示して組み込みデフォルトを使います。値が 1 つだけ不正な場合（`age = "abc"` など）は、警告を表示し、そのキーだけを組み込みデフォルトに戻します。

## バージョン指定と書き換え

この章では、すべてのエコシステムに共通する規則を説明します。どの指定を固定とみなすか、範囲をどう進めるか、どの形式を保つか、どの制約を書き換えないか、候補をどう順序付けるか、マニフェストをどう書き込むかです。1 つのエコシステムだけに関わる規則は「[エコシステム別の詳細](#エコシステム別の詳細)」にまとめています。
...
> **注意**: Go と mise は例外で、完全一致のバージョンも `--include-pinned` なしで更新します。Go の依存を据え置くには、その行に `// pinned` コメントを付けます（[Go](#go)）。mise のツールを据え置くには `--exclude <ツール名>` を指定します。名前はすべての言語で照合されるため、たとえば `--exclude node` は npm の `node` パッケージも除外します。
...
npm / Composer のハイフンレンジで、`1.0 - 2.0` のように右辺が部分指定のときは、右辺の `2.0` を `2.0.x` とみなし、排他的な上限 `<2.1` として解釈します。そのため `2.0.x` は候補に残り、`2.1.0` 以降は候補から外れます。
...
depup が書き換えるのは、解析した依存宣言だけです。それ以外のセクションは、パッケージ名やバージョンが書かれていても変更しません。たとえば `package.json` の `overrides`、`composer.json` の `replace` / `provide` / `conflict`、`Cargo.toml` や `pyproject.toml` のメタデータのテーブルがこれにあたります（対象のセクションは[エコシステム別の詳細](#エコシステム別の詳細)を参照）。値を書き換えるときは、周囲の書き方も保ちます。TOML マニフェストでは、基本文字列（`"..."`）とリテラル文字列（`'...'`）のどちらも、引用符の種類を変えません。

同じ依存キーがマニフェスト内で複数回宣言されている場合や、複数の Gradle 依存が 1 つのバージョン変数または version catalog の `version.ref` を共有している場合、depup はどの宣言を書き換えるべきか判断できません。この場合は書き込みを拒否し、エラーとして報告します（終了コードは 2）。固定された宣言のように変えてはいけない宣言を、黙って書き換えないためです。
...
install が失敗しても、残りの install は続けます。失敗したコマンドとパッケージマネージャーの標準エラー出力を表示し、最後に `Error: Some package manager installs failed` を出して終了コード 1 で終わります。パッケージマネージャーがインストールされていない場合も失敗として扱います。書き換え済みのマニフェストは元に戻しません。また、どのプロジェクトでも Rust の監査（[後述](#推移的依存と-age-フィルター)）を実行しません。
...
表のファイルがどれもなければ、その言語の install は実行せず、何も表示しません。age フィルターが有効なときは、pnpm・uv・mise に age の値も渡します（[後述](#推移的依存と-age-フィルター)）。`uv sync` には常に `UV_MALWARE_CHECK=1` を付けます（[uv のマルウェアチェック](#uv-のマルウェアチェックpreview)）。
...
| mise | 環境変数 `MISE_MINIMUM_RELEASE_AGE=<秒>s` | mise のツールに推移的依存はない。`node = "26"` のような部分指定を `mise install` が解決するときに age を適用する |
...
- `✓ OSV` は、脆弱性チェックを通過したバージョンに付きます。更新行の下の `↳ OSV skipped:` 行は、脆弱性があるため候補から外したバージョンです（依存そのものは更新されています）。
- スキップした依存は、マニフェストの見出し（`— N updates, M skips`）と Summary に件数を表示します。更新が 0 件のマニフェストでは、理由ごとの件数も表示します。
...
終了コード 2 の原因があっても、処理は止まりません。取得に失敗した依存はスキップし、読み込みや解析に失敗したマニフェストは処理から外して、それ以外の書き込みや install は続けます。書き込みに失敗したファイルは元のまま残りますが、その依存は更新として表示され、`--install` も実行されます。`1` と `2` の両方に当てはまる場合は `1` になります。取得の問題のうち次の 3 つは、終了コードを変えません。Cargo の git 依存の `git ls-remote` の失敗、使えるバージョンをレジストリが 1 つも返さない場合（`fetch failed: no versions available`）、`mise` コマンドがない場合（mise の設定ファイルを警告付きで読み飛ばす）です。「更新がある」ことを示す専用の終了コードはないため、CI で結果を調べるには `--json` を使ってください。
...
依存が出力にまったく現れない場合、depup はその宣言を対象外にしています。ファイル・セクション・宣言の書き方が対応しているかを、[エコシステム別の詳細](#エコシステム別の詳細)で確認してください。`--node` のような言語の指定から外れた言語のマニフェストは解析しません（pnpm・Bun・mise のプロジェクトの age 設定は読み取ります）。マニフェストは更新されたのに install が失敗した場合は、[パッケージマネージャーの実行](#パッケージマネージャーの実行--install)を参照してください。
...
PyPI 以外をデフォルトのインデックスにしている `pyproject.toml` では、すべての依存を対象外にし、警告を 1 回表示します。該当するのは、Poetry の `priority = "primary"` / `"default"` の source、uv の `[[tool.uv.index]] default = true` や `[tool.uv] index-url`、PDM による `pypi` の上書きです。depup は PyPI しか参照しないため、更新すると非公開パッケージが同名の公開パッケージに置き換わってしまいます。
...
git 依存は、レジストリの代わりに `git ls-remote` で確認します。`tag` は、最新の安定版の SemVer タグへ更新します。そのタグが `--max-change` の上限を超える場合は `max-change=<LEVEL>` としてスキップし、上限内の古いタグは候補にしません。`branch`（省略時はデフォルトブランチ）は、リモートの先頭のコミットが `Cargo.lock` に記録されたコミットと異なるとき、または記録がないときに、更新として報告します。このとき `Cargo.toml` は書き換えず、新しいコミットは `--install` で実行する `cargo update` がロックファイルに取り込みます。`--install` を付けなければ何も書き込みません。`rev` は、`--include-pinned` を付けても常に `pinned` としてスキップします。`tag` の書き換えは、バージョンの書き換えと同じ依存テーブル（inline table と複数行テーブルの両方）に限り、単一引用符・二重引用符を保ちます。`[patch.<registry>]` / `[patch.<registry>.<package>]` の `tag` も更新します。git 依存には age フィルターも OSV チェックも適用しません。`git ls-remote` に失敗した依存はスキップするだけで、終了コードは変えません。
...
Go の `exclude` ディレクティブに書かれたバージョンは、候補から外します（`exclude` の記述自体は書き換えません）。上流モジュールの最新の `go.mod` が `retract` したバージョン（単一のバージョンでも閉区間でも）も候補から外します。タグ付きのバージョンがないモジュールでは Go Proxy の `@latest` へフォールバックします。`.info` の `Time` が省略されている場合は Unix エポックを公開日時として扱うため、公開日が不明なバージョンが `--age` で永久に除外されることはありません。

Go の `+incompatible` 付きバージョンは、候補に残すかどうかを `go` コマンドと同じ規則で決めます。SemVer の順序で最初の `+incompatible` に達したとき、直前の compatible なバージョンに本物の `go.mod`（Go Proxy が合成したものではないもの）があれば、それ以降の `+incompatible` をすべて候補から外します。この処理がないと、`github.com/libp2p/go-libp2p` が `v0.49.0` から 2018 年の `v6.0.23+incompatible` へ「更新」され、しかもビルドは通るため、誤りに気づけません。
...
`composer.json` で更新するのは `require` と `require-dev` です。`replace` / `provide` / `conflict` などのセクションは変更しません。明示的な等価演算子（`=1.2.3` / `==1.2.3`）は、演算子を保ったまま更新します。「等しくない」を `<>` で書いた除外制約（`<>1.2.3` など）は、解析しますが書き換えず、スキップします。
...
JVM 系の milestone 版はプレリリースとして扱い、安定版の依存では候補から外します。対象は `4.0.0-M1`、旧 Spring Boot のドット区切り `2.0.0.M1`、省略しない綴りの `-milestone1` です。この扱いがないと、`assertj-core 3.24.2` が `4.0.0-M1` へ、`junit-bom 5.10.0` が `5.13.0-M3` へ、`spring-core 5.3.23` が `7.0.0-M6` へ更新されてしまいます。判定は「`m` の直後が数字」のトークンに限るため、JVM の安定版の qualifier（`.Final`、`-jre`、`-android`、`.RELEASE`、`.GA`、`-SP1`）や `-macos1` のような識別子を誤判定しません。他のプレリリースと同じく、現在のバージョンが milestone の場合は候補に milestone を残すため、次の milestone へ進めます。

### Swift

Swift の GitHub 依存では、HTTPS の URL、scp 形式の SSH URL（`git@github.com:owner/repo.git`）、標準の SSH URL（`ssh://git@github.com/owner/repo.git`）、GitHub の SSH over 443 の URL（`ssh://git@ssh.github.com:443/owner/repo.git`）を解析できます。GitHub のタグとしては `v1.2.3` と `V1.2.3` の両方を認識しますが、`Package.swift` の version requirement 文字列は厳格な SemVer（`X.Y.Z`、先頭ゼロなし）として検証します。また、`Package.swift` の `//` 行コメントや `/* ... */` ブロックコメントの中に書かれた依存宣言は無視します。SwiftPM が Semantic Versioning 2.0.0 に従うのに合わせて、プレリリース識別子付きのバージョン（`1.0.0-beta.1`）、ビルドメタデータ付きのバージョン（`1.0.0+build.123`）、両者を組み合わせた形式（`1.0.0-rc.1+sha.abc`）も解析・更新できます。`.package(...)` の末尾に `traits: [...]`（SPM 6.1 の Package Traits）や `moduleAliases: [...]` のような追加の引数があっても、version requirement だけを置き換えて追加の引数は保持します。Swift Package Registry の `id:` 依存（`.package(id: "scope.name", ...)`）には現在対応しておらず、対象外です。registry API のアダプターが未実装のためで、将来対応する予定です。Swift で対象になるのは GitHub URL の依存だけで、GitHub 以外の URL や `branch:` / `revision:` の指定も対象外です。
