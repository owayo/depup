...
- 安全に書き換えられない制約（例: npm/Composer の `^1 || ^2`、`!=` を含む除外制約 `!=1.2.3` / `>=1.0, !=1.5.0, <2.0`、上限のみの `<4.0.0` / `<=2.0`、厳密な下限の `>1.0.0`、下限なし Maven 形式 `(,2.0]`、排他的下限を持つ Maven 形式 `]1.0,2.0[` / `]1.0,2.0]`）は自動更新から除外される。Composer (composer/semver) は not-equal を `!=` と `<>` の両方で綴れる (演算子パターン `(<>|!=|>=?|<=?|==?)`) ため、`>=1.0 <>1.5.0 <2.0` / `>=1.0,<>1.5.0,<2.0` のような `<>` 除外制約も `!=` と同様に安全側でスキップする (`contains_not_equal_operator` が両綴りを検出。以前は `<>` だけ下限を書き換えて、除外バージョンを選ぶと `>=1.5.0 <>1.5.0 <2.0` のような充足不能な制約を書き戻す恐れがあった)
