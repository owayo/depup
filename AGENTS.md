...
src/
  ...
  manifest/
    ...
    mise.rs          - mise マニフェストの形式振り分け (TOML 形式 / .tool-versions)
    mise_toml.rs     - mise.toml の [tools] パーサ
    tool_versions.rs - asdf 互換 .tool-versions パーサ
    mise_settings.rs - mise の [settings] minimum_release_age 読み取り
  parser/           - 言語別パース処理
  registry/
    ...
    mise_registry.rs - mise ls-remote --json でツールのバージョン一覧を取得
  update/
    filter.rs        - フィルタ設定
...
| mise | mise.toml / .mise.toml / mise/config.toml / .mise/config.toml / .config/mise.toml / .config/mise/config.toml / .tool-versions | `mise ls-remote` (子プロセス) |
...
- 更新候補の比較はエコシステム別に行う。Node/Rust/Go/Swift は SemVer 2.0.0 を使い、純粋に数値だけの `-` サフィックス (`1.0.0-1`) もプレリリースとして安定版より小さく扱う。優先順位の比較では `semver::Version::cmp_precedence` を使って build metadata (`+...`) を無視し、`1.1.3` と `1.1.3+spec-1.1.0` の差だけで更新を発生させない。通常の `Ord` は全順序のため build metadata も比較するので更新判定には使わない。Python は `pep440_rs` による PEP 440 正規化・順序付け（代替綴り、epoch、pre/dev/post/local を含む）、Ruby は RubyGems の数値・英字セグメント順を使い、英字またはハイフンを含む版をプレリリースとして安定版利用者の候補から除外する。PHP は composer/semver の modifier 正規表現 (`[._-]?(stable|beta|b|RC|alpha|a|patch|pl|p)((?:[.-]?\d+)*)([.-]?dev)?`) をそのまま実装して正規化する。**区切り文字は省略可能で `.` / `_` も使える**ため、`5.0.0alpha3` / `1.0.0.RC1` / `1.0.0_beta1` は prerelease、`2.2.1p1` / `2.2.1pl1` / `2.2.1patch1` は patch alias (post = 安定版より新しい) として扱う。以前は `rsplit_once('-')` でハイフン区切りだけを見ていたため、実在パッケージで 2 つの事故を起こしていた: `nikic/php-parser` の `5.0.0beta1` が安定版と判定されて `>=4.0 <5.0.0` の利用者へ `>=5.0.0beta1 <5.0.0` (minimum-stability: stable では解決不能) を書き込み、`laminas/laminas-diactoros` のセキュリティパッチ `2.2.1p2` が `2.2.1` より小さいと判定されて `>=2.2.1p2 <2.2.1` (完全な空集合) を書き込んでいた。短縮綴り `a` / `b` / `p` / `pl` は数字が続くときだけ modifier として展開し、`1.0.0-arm64` / `1.0.0-alpine` を巻き込まない。`stable` は本家 `normalize` と同じく修飾子を落とす (`1.0.0-stable == 1.0.0`)、Java は Gradle 公式の version ordering（区切り同値、英数字分割、追加パート、special qualifier 順）を使う。数値コア・epoch・pre/post/local の数値識別子は `u64` へ変換せず任意桁の10進文字列として比較し、巨大な数値でもオーバーフローによるダウングレードを起こさない
...
- Gemfile のバージョンなし `git:` / `github:` / `bitbucket:` / `gist:` / `path:` / `source:` 依存は RubyGems のレジストリ依存ではないため更新対象から除外する。バージョンが明示されている同種依存は Bundler の gemspec バージョンチェックとして解析・更新対象にでき、source オプションは保持する。対象キーは Bundler が組み込みで登録する git source ショートハンド (`dsl.rb` の `add_git_sources` が登録する `github` / `gist` / `bitbucket` / `gitlab`。`gitlab` は Bundler 2.5.7 で追加) と `git` / `path` / `source` で、`NON_REGISTRY_OPTION_KEYS` に集約して行オプション形式とブロック形式で共有する。オプションキーは Ruby の 2 通りの綴り (`git: '...'` と `:git => '...'` の hash-rocket 記法) の両方を検出する (以前は `git:` 綴りしか見ておらず、git 依存を更新候補として報告した上で書き込み時に必ず失敗していた)
...
- Gemfile の複合制約 (`gem "pg", ">= 0.18", "< 2.0"`) は解析・更新の両方に対応する。judge が使う `format_range_like` の結果 (`">= 1.5.9, < 2.0"`) をカンマで分割し、元の引数の個数・順序・クォート種別・引数間の空白・括弧・行末修飾子へ配り直して書き戻す。分割結果の要素数が元の引数の個数と一致しない場合 (引数自身がカンマを含む `'>= 0.18, < 1.0'` 等) は安全のためエラーにする。除外制約 (`'!= 2.2.4'`) は `format_range_like` が `None` を返すため judge の段階で Skip される。以前は writer が複数引数を無条件にエラーにしていたため、Rails + PostgreSQL の標準 Gemfile (`gem "pg", ">= 0.18", "< 2.0"`) で「1 updated」と報告しながら 1 バイトも書けず exit code 2 になっていた
- Gemfile の複合制約の比較基準は記述順に依存せず包含下限を採用する (`gem 'pg', '< 2.0', '>= 0.18'` でも基準は `0.18`)。他言語と同じ `range_lower_bound_version` を共有するため、judge の比較基準と writer の書き換え対象が必ず一致する
- Bundler の `git_source(:name) { ... }` で登録したカスタムショートハンド (`gem 'rails', stash: 'forks/rails'`) も非レジストリ依存として更新対象から外す。宣言はファイル走査の 1 パス目で集め、parse と `update_version` が同じ集合を共有する。組み込み 7 キーだけを見ていたときは、git チェックアウト先の gem に rubygems.org の版を注入して `bundle install` を壊していた
- `update_version` も parse と同じブロック追跡を行い、`path ... do` / `git ... do` ブロック内の gem は書き換えない。parse がブロック内の宣言を捨てるため writer の曖昧性ガードも効かず、同名 gem がブロック内外にある構成 (ローカル開発切り替え) でローカル path 依存にレジストリの版が注入されていた
...
- comparator set / カンマ区切り複数要件に**埋め込まれた** tilde もセグメント数を保つ (`~1.2 <2.0.0` → `~1.9 <2.0.0`、Cargo の `~1, <5.0` → `~4, <5.0`、Composer の `~1.2 <2.0` → `~1.9 <2.0`)。単体の Tilde は `format_tilde_like` が保護するが、上限が付いて `VersionSpecKind::Range` になった途端に `format_range_like` が完全版を埋めており、`~1` (上限 `<2.0.0`) が `~1.9.3` (上限 `<1.10.0`) へ黙って縮んでいた。判定は `find_inclusive_lower_bound_token` が返す演算子で行い、PEP 440 の `~=` と同じ `replace_version_token_preserving_shape` 経路へ寄せる。`^` は上限がセグメント数に依存しない (`^1` も `^1.9.3` も上限は `<2.0.0`) ため対象外
...
- 先頭ゼロの数値 prerelease 識別子を判定する前に build metadata (`+...`) を切り落とす。SemVer 仕様上 build 識別子はハイフンを含んでよく先頭ゼロも許されるため (`1.2.3+00` / `1.0.0+2024-01` はいずれも valid)、`-` を先に探すと build 部を prerelease と誤認する。以前は `1.0.0+2024-01` のような日付入り build metadata を持つ版が parse 時点で丸ごと None になり、その依存が報告にも更新にも現れず無言で取りこぼされていた
...
- Cargo.toml の `registry = "..."` 付き依存は、`crates-io` 以外のレジストリであれば crates.io の候補で誤更新しないよう更新対象から除外する。インデックス URL を直接指す `registry-index = "..."` も同様に除外する (値に関わらず crates.io 以外を指す。Cargo は `registry` との同時指定を拒否する)。`registry` 用のパターンは `registry\s*=` を要求するため `registry-index` にはマッチせず、parse・writer とも素通りして社内 private registry の依存が同名の公開クレート (typosquat を含む) の版で書き換えられていた。inline table / 複数行テーブル / dotted key の 3 形式すべてで除外する
...
- Swift の GitHub URL は HTTPS、scp 形式 (`git@github.com:owner/repo.git`)、標準 SSH (`ssh://git@github.com/owner/repo.git`)、GitHub の SSH over 443 (`ssh://git@ssh.github.com:443/owner/repo.git`) を owner/repo へ正規化する。`github.com.evil` 等の類似ホストは拒否する。**書き換え側の URL 正規表現も同じ形式を受理する必要がある** — parse は正規化ベース、update は生 URL の正規表現という二重定義のため、ホスト部に `ssh.` 接頭辞とポート番号 (`(?:ssh\.)?github\.com(?::\d+)?`) を許さないと SSH over 443 だけ parse は通るのに書き換え先が見つからず report/apply が矛盾する
...
- Maven Central は `numFound` を読んで `start` でページングし、最大 10 ページ (2000 版) まで取得する。`core=gav` は timestamp 降順で返すため 1 ページ (旧実装は `rows=100`) で打ち切ると「最新 100 版」しか得られず、古い系列に固定した依存 (`[1.11,1.12)` / `1.11.+` / `--max-change patch`) の後継版が候補に 1 件も入らず AlreadyLatest と誤判定して無言で取りこぼしていた (`org.slf4j:slf4j-api` は numFound 106、`com.amazonaws:aws-java-sdk-s3` は 1955)
...
- age が有効な場合 (CLI `--age` / プロジェクト `minimumReleaseAge` / グローバル設定 / 組み込みデフォルト 1w のいずれか) の transitive 依存への適用方法は PM ごとに異なる。install フェーズも judge と同じ解決済み age を使うため、CLI `--age` を明示しなくてもプロジェクト設定やデフォルト 1w が transitive へ反映される:
  - **Rust (cargo)**: `cargo update` 後に Cargo.lock を走査し、age 違反を `cargo update -p <name> --precise <older_version>` で age 内の最新 stable バージョンへ差し戻す (post-install audit)。cargo の再解決で連鎖する新たな違反に備えて最大 5 回まで反復する。resolver 制約違反で差し戻し不可の場合は verbose でスキップ理由を表示して続行。**監査対象は install 前後で lock が変わったエントリだけに絞る** (`changed_entries`) — install 前の Cargo.lock を控えておき、同じバージョンのまま残っているものは depup が入れたものではないので照会しない。crates.io は crawler policy により 1 リクエスト/秒へ直列化されるため、lock 全体 (depup 自身なら 249 件) を舐めると監査だけで 4〜9 分かかり、進捗表示のない無音待ちがハングと区別できなかった (実測で監査対象 249 件 → 24 件)。install 前に Cargo.lock が無ければ baseline は空 = 全エントリが新規なので従来どおり全件監査になる。安全弁として監査には `LOCK_AGE_AUDIT_BUDGET` (180 秒) の時間予算があり、Cargo.lock の有無に関係なく常に効く (タイマーは lock ディレクトリごとに監査開始時から数えるので、lock が複数あれば各 180 秒)。超過時は監査済みの差し戻しを保持したまま打ち切り、未検証件数を stderr に警告する (exit code には影響しない)。install が 1 件でも失敗した場合、監査自体を実行しない。差し戻しで起動する `cargo update -p --precise` は 1 回 `CARGO_UPDATE_TIMEOUT` (120 秒) で打ち切る (`kill_on_drop` 付き)
...
- グローバル設定ファイル `~/.config/depup/config.toml` は初回実行時に自動生成される (claw-hooks 方式)。雛形は組み込みデフォルトと一致するキーをコメント付きで書き出す (`age = "1w"` / `osv = true`、オプトインの `max_change` はコメントアウト)。生成失敗時 (権限など) は警告を出して処理を継続し、組み込みデフォルトで動作する。既存ファイルは絶対に上書きしない
...
- OSV.dev API による脆弱性チェックは**組み込みデフォルトで有効** (`DEFAULT_OSV = true`)。設定ファイルが無い場合も、設定ファイルに `osv` キーが無い場合も有効。`--no-osv` またはグローバル設定 `osv = false` で無効化する。`judge_with_osv` は採用しようとした候補だけを `https://api.osv.dev/v1/query` に POST し、`vulns` が空でなければ その version を candidate から除外して再 judge → 次に古い候補へフォールバックする。全 candidate を網羅的にチェックする方式は採らない (1000+ バージョンを持つ `@angular/*` 等で実用速度を確保するため)。通常 1 依存あたり 1〜2 API call で完了する。API エラー (送信失敗・2xx 以外・JSON 解析失敗) 時は judge が選んだ候補をそのまま採用し (検査済みではないので `✓ OSV` は付かない)、警告を `OsvWarning` として errors に積む。この警告は `--verbose` の有無に関係なくテキスト出力の `Errors:` 欄と JSON の `errors` に載り (`--diff` / `-q` では出ない)、exit code には影響しない。stderr の `⚠ OSV check failed for ...` 行は進捗バー (`osv_println`) 経由のため、stderr が TTY のときだけ見える (CI やリダイレクトでは indicatif がバーごと隠す)。以前の記述「API エラー時は `--verbose` で警告」は誤りで、README も同じ誤りを直した。Swift / mise / Cargo の git 依存は OSV を通さない。優先順位は `--no-osv` > `--osv` > グローバル設定 > 組み込みデフォルト (true)。OSV API は認証トークン不要。脆弱版除外は言語別の `compare_dependency_versions` を使うため、Python の PEP 440 ローカルバージョン (`1.0+cu121` 等) が build metadata を無視する semver 比較で誤って同列と判定され、安全な候補まで NoSuitableVersion に落ちることがない
...
- Composer は `~>` を拒否する (composer/semver の tilde 分岐は正規表現 `{^~>?<version>$}i` にマッチした**後**に `Invalid operator "~>"` を throw する) ため、PHP パーサは `~>` を parse 時点でスキップする。受理すると「更新しました」と報告しつつ composer が読めない制約を書き戻すことになる。node-semver は `LONETILDE = (?:~>?)` で `~>` を valid とするので Node パーサは従来どおり受理し、更新後も綴りを保つ (言語間で挙動が違うのは仕様差によるもの)
...
- HTTP のハング対策は多層で持つ: reqwest 側にリクエスト単位の `timeout` (30秒) に加えて `connect_timeout` (10秒) と `tcp_keepalive` (60秒) を設定し、さらにリトライループ全体を `tokio::time::timeout` の `TOTAL_DEADLINE` (120秒) で切る。reqwest の `timeout` は 1 回の送受信にしか掛からないため、これが無いとリトライを使い切るまでの合計 (最悪 2 分超) が実質の上限になり、接続プール内の半死コネクションを掴んだケースも拾えない。`connect_timeout` は経路が塞がれた環境 (プロキシ配下・VPN 切断直後) で応答の来ない SYN を全体タイムアウトいっぱい待たされるのを防ぐ。`tcp_keepalive` は crates.io のように 1 リクエスト/秒 で数十秒アイドルする接続が経路上の NAT / LB に黙って切られたまま残るのを防ぐ。OSV クライアントも同様に `connect_timeout` (5秒) を持つ
- 長時間かかるフェーズは必ず進捗を出す (ネットワーク待ちの無音は利用者からハングと区別できない)。`--install` のパッケージマネージャ実行は出力をキャプチャするため完了まで無表示になるので言語ごとにスピナーを出し、Rust の post-install age 監査は `{pos}/{len}` 付きバーで「今どの crate を照会中か」「どの crate を差し戻し中か」を表示する。バー表示中の警告・詳細ログは `Progress::suspend` で包み、バーと出力行が混ざらないようにする
...
- マニフェストの書き込みは同一ディレクトリの一時ファイル + rename によるアトミック置換 (途中失敗で部分内容が残らない)。rename の前に一時ファイルを `sync_all` で永続化し、rename 後はディレクトリエントリも best-effort で fsync する — `fs::write` はページキャッシュへ書くだけなので、fsync しないと rename のメタデータだけが先に永続化され、電源断でゼロ長または部分内容のマニフェストが残りうる (元の内容も失われる)。一時ファイル名には PID に加えてナノ秒を混ぜ、`OpenOptions::create_new(true)` で作成する — 名前が PID だけだと予測可能で、同名の symlink を先に置かれると `File::create` (`O_CREAT|O_TRUNC`、`O_NOFOLLOW` なし) がリンク先へ書き込み、続く `set_permissions` がリンク先を chmod し、`rename` がマニフェストを symlink に化けさせる。`create_new` は既存 (symlink 含む) があれば EEXIST で失敗するのでこの経路を塞ぐ。既存ファイルのパーミッションは引き継ぎ、読み取り専用ファイルへの書き込みは従来どおりエラー (書き込み可否は rename 前に `OpenOptions::write(true).open` で明示的に検査する)。マニフェスト自体が symlink の場合は `canonicalize` でリンク先の実体へ解決し、実体側のディレクトリで一時ファイル + rename を行う (rename(2) は symlink 自体を置換してしまうため。これによりリンク先 (モノレポの共有マニフェスト等) を更新し symlink 構造を保つ。tmp も実体側に作るため同一ファイルシステム内に収まり EXDEV にならない)
- `--diff` の hunk 見出しと before/after 行にはマニフェスト上の依存キー (`manifest_name()`) を使う。`dependency.name` はレジストリ上の実パッケージ名なので、npm alias (`"react": "npm:@preact/compat@^17"`) や Cargo の `package = "..."` リネーム依存では実ファイルに存在しないキーが並ぶ
- スキップ理由の集計表示は件数降順 + 理由名の 2 段ソートで順序を確定させる。`HashMap` の `RandomState` はインスタンスごとにシードが変わるため、同数の理由が複数あると安定ソートでは実行のたびに行順が入れ替わり、CI のスナップショット比較で偽の差分になる
- `(dry-run)` 接頭辞はマニフェスト見出しの全経路に付ける (更新 0 件 + スキップありの見出しだけ落ちていると、両方が混在するプロジェクトで一部の行にしか付かない)
...
- `--age` を transitive 依存へネイティブに適用できる言語かどうかは `Language::has_native_transitive_age_support()` が単一の情報源 (Node = pnpm の env var / Python = uv の `--exclude-newer` / Rust = post-install lock 監査 / mise = `mise install` へ渡す `MISE_MINIMUM_RELEASE_AGE`)。`match` の網羅性検査が効くため、言語を追加したときに `--verbose` の通知漏れが起きない
- mise (`Language::Mise`) のバージョン一覧は HTTP レジストリではなく `mise ls-remote <tool> --json --minimum-release-age 0` の子プロセス実行で取得する。mise は core / aqua / ubi / asdf / npm: / cargo: / go: / pipx: など多数のバックエンドを束ねており、depup が各配布元を直接叩くのは非現実的なため。`--minimum-release-age 0` は必須で、env `MISE_MINIMUM_RELEASE_AGE=0` も併せて渡す (mise 側の既定 24h が効いたままだと新しい版が最初から隠れ、depup の age 判定と二重になる)。age の適用は depup 側に一本化する。60 秒タイムアウト・`kill_on_drop`・ツール名の文字種検証 (`-` 始まりを拒否、`--` で引数終端) を付ける。`mise` コマンドが PATH に無ければ mise マニフェストを丸ごと外して警告を 1 回だけ出す (依存の数だけ同じ fetch エラーを並べない)
- mise の候補には同じツールでもベンダー違いの版が混ざる (`mise ls-remote java` は 3000 件超のうち 3136 件が `temurin-` / `graalvm-community-` / `zulu-` などの接頭辞付き)。接頭辞を無視して最新を選ぶと `temurin-21` の利用者が `zulu-27` へ飛ぶため、`split_mise_flavor` で「`-` の直後が数字になる**最初**の位置」を境界に接頭辞と数値部を分け (`temurin-21.0.5-b1` を `temurin-` / `21.0.5-b1` と切る。最後の `-` で切ると `b1` 側に割れる)、judge 前に `mise_flavor_candidates` で同じ接頭辞の候補だけへ絞って数値部へ正規化する。書き戻しは `format_mise_version` がマニフェスト上の元表記から接頭辞を復元するため、judge が扱う値に接頭辞が無くても失われない。同じ接頭辞の候補が 1 件も無ければ `NoSuitableVersion` で止める (別ベンダーへ乗り換えない)
- mise の部分指定 (`node = "26"` / `"26.7"` / `"prefix:26"`) は `VersionSpecKind::Prefix` として扱い、更新後も**元のセグメント数**を保つ (`26` → `27`、`26.7` → `26.8`)。セグメント数がそのまま「どこまで固定するか」を表すため、レジストリの完全版を書き戻すと `node = "26"` (26 系の最新を都度解決) が完全ピンへ黙って変わる。数値セグメントが 3 以上、または数値の後ろに識別子がある版 (`3.15.0rc1` / `3.15-dev`) は `Exact`。`prefix:` セレクタが明示されていれば常に `Prefix`
- mise の `latest` / `lts` / `system` / `ref:` / `path:` / `sub-N:` / 配列指定 (`python = ["3.12", "3.13"]`) / `.tool-versions` の複数バージョン指定は parse 時点でスキップする。浮動指定は更新しても意味が変わらず、配列はどの要素を更新すべきか決められない (Poetry のマルチ制約配列と同じ安全側のスキップ)
- mise のバージョン比較は `compare_gradle_versions` を流用する。バックエンドごとに体系が違い semver とは限らない (java の `26.0.2.1`、python の `3.15.0rc1` / `3.15-dev`) ため、「数値と英字を分割し、数値は数値として、qualifier は既知の順序で比較する」Gradle の version ordering が最も破綻しない
- mise の対象ファイルは `mise.toml` / `.mise.toml` / `mise/config.toml` / `.mise/config.toml` / `.config/mise.toml` / `.config/mise/config.toml` / `.tool-versions`。`mise.local.toml` / `.mise.local.toml` (個人のローカル上書き。通常 gitignore) と `mise.<env>.toml` (環境別 overlay) は意図的に対象外。形式の判別は内容ベースで、`looks_like_tool_versions` が「全ての意味のある行が `<tool> <version>` 形式」のときだけ `.tool-versions` と認め、`[` で始まる行や `=` を含む行が 1 行でもあれば TOML 扱いにする (壊れた TOML を `.tool-versions` と誤認して黙って読み飛ばさない)
- `mise.toml` の書き換えは `[tools]` セクション限定。`[settings]` / `[env]` / `[tasks]` / `[alias]` の同名キーは触らない。対応する記法は `node = "26.7.0"` / `"npm:prettier" = "3.9.6"` (クォート付きキー) / `node.version = "26.7.0"` (dotted key) / `java = { version = "...", postinstall = "..." }` (inline table) / `[tools.terraform]` + `version = "..."` (テーブル形式) / `[tools."npm:prettier"]` (クォート付きセクションヘッダ)。dotted key は toml クレートが inline table と同じ構造へ畳むため parse が surface する。`:` を含むツール名はテーブルヘッダでクォートが必須なので、セクションキーは `split_section_key` でクォートを剥がしてから比較する。どちらも書き換え側が対応しないと「更新あり」と報告した後に書き込みが失敗して report/apply が矛盾する
- inline table の `version` 置換は、正規表現の一致位置が**別フィールドの文字列リテラルの内側**にある場合は飛ばす (`string_literal_ranges` で範囲を列挙してから判定)。`java = { postinstall = 'echo version = "x"', version = "temurin-21" }` では `postinstall` の中身が先に一致するため、素朴に最初の一致を置換するとツールのバージョンではなくコマンド文字列を破壊する
- mise の `[settings] minimum_release_age` が**明示的に書かれている**場合のみ、pnpm / bun の `minimumReleaseAge` と同じプロジェクトポリシーとして CLI `--age` を上書きする (mise 既定の 24h は「未設定」。`mise settings get minimum_release_age` も未設定ならエラーを返す)。単位は humantime 準拠で **`m` は分**であり、depup CLI の `--age 1m` (1 か月) とは意味が違うため `parse_mise_duration` を専用に持つ。`2024-06-01` のような絶対日付は「現在からの経過時間」に変換できないので採用しない。`minimum_release_age_excludes` は depup 側で解釈しないため、設定されていれば警告を出す (mise では age 対象外のツールが depup では age 適用され、report と `mise install` の結果が食い違いうる)
- npm/Composer のハイフンレンジと `<=` の上限抽出は端点のワイルドカード (`1.2.x - 2.3.x` / `<=2.x`) も解釈する。判定用トークン `WILDCARD_BOUND_TOKEN` はワイルドカードセグメントを許し、`strip_wildcard_tail` でワイルドカード以降を落としてから「最終セグメント +1 の排他的上限」へ展開する (node-semver と同じく `2.3.x` は `<2.4.0`、`2.x` は `<3.0.0`)。パーサ側が x-range 端点を Range として受理するのに上限抽出が数値セグメントしか読めなかったため、上限フィルタが丸ごと効かず、judge がレジストリ最新版を選び writer が下限だけを置換して `4.17.x - 2.3.x` (= `>=4.17.0 <2.4.0-0`、**空レンジ**) を書き戻し `npm install` を壊していた。排他側 (`<2.x` → `<2`) は `<2.0.0` と等価なので従来どおり
- comparator set に埋め込まれた tilde の下限がプレリリース識別子やビルドメタデータを含む場合 (`~1.2.3-rc.1, <2.0`)、セグメント数を保てないので完全版へフォールバックする。単体 Tilde は `format_tilde_like` が同じ状況で完全版を使うため、上限が付いて Range 扱いになった途端に書き換え不能になる非対称があった
- OR 結合された制約は `|` 1 文字で判定して書き換えを拒否する。composer/semver の `parseConstraints` は `preg_split('{\s*\|\|?\s*}')` で `|` と `||` を同格の OR として扱うため、`||` だけを見ていると `>=1.0 <2.0 | >=3.0` のような後方互換表記が素通りし、片方の分岐の下限だけを進めて union の意味を変えていた (`^1 | ^2` が弾けていたのは `format_partial_version_like` の数値セグメント検査に引っかかる副作用にすぎない)
- Go は `@v/list` の `+incompatible` を Go 本体の `modload.filterVersions` と同じ規則で除外する。semver 昇順で最初の `+incompatible` に到達したとき、直前の compatible 版が**本物の** `go.mod` (合成された `module <path>` 1 行ではない) を持つなら以降の `+incompatible` を全部捨てる。proxy の `@v/list` は 2019 年の修正前にキャッシュされた `+incompatible` を保持し続けるため、クライアント側のこのフィルタが唯一の防波堤で、無いと `github.com/libp2p/go-libp2p v0.49.0` が 2018 年の `v6.0.23+incompatible` へ「更新」される (`go build` は通るので気づけない)。retract 情報源もフィルタ後の一覧から選ぶ (合成 go.mod を情報源にすると本物の retract を取りこぼす)
- Go の `replace` は対象モジュールを `require` 側と突き合わせて更新対象から外す。版なし replace は対象モジュール全体、版付き replace は版が一致する `require` だけを除外する (Go の `replacement()` と同じ規則)。`require` だけ更新すると版付き replace の LHS が一致しなくなり、ローカルのパッチが黙って外れて上流コードに切り替わる
- go.mod の行末コメントは空白なし (`v1.0.0//indirect`) でも認識する。go の字句解析は識別子中の `//` で必ず切るため正当な構文で、バージョン部のパターンを `[^\s]*` から `[^\s/]*` にして境界を作った
- Go proxy の `@v/{version}.info` 取得は `buffer_unordered` で並列化する (`INFO_FETCH_CONCURRENCY = 8`)。`@v/list` は数千件になりうる (`github.com/aws/aws-sdk-go` で 1865 件) ため、直列に引くと 1 依存の解決だけで数分の無音待ちになり、利用者からはハングと区別がつかない。ストリームには所有値 (`String` の URL) を流す — `&str` を流すとクロージャが特定のライフタイムに束縛され `buffer_unordered` が要求する高階ライフタイム境界を満たせない
- Gradle の `resolutionStrategy` / `constraints` / `dependencySubstitution` ブロック配下の座標は依存として surface しない。`DEP_STRING` は任意の語を configuration 名として受理する (カスタム configuration 名を通すための設計) ため、`force 'g:a:v'` も依存として拾って直接宣言と重複し、writer の曖昧性ガードが更新を拒否して exit code 2 になっていた。ブロックを開かない `resolutionStrategy.force 'g:a:v'` 用に `force` は configuration 名からも除外する
- Gradle の `-SNAPSHOT` / `.SNAPSHOT` で終わる固定指定 (`1.2.3-SNAPSHOT` / `1.2.3-SNAPSHOT!!` / `[1.2.3-SNAPSHOT]`) は更新対象から外す。SNAPSHOT は rc/beta のような固定プレリリースではなく**毎解決ごとに最新タイムスタンプ版を取り直す浮動参照**なので、`latest.release` と同じクラスとして扱う。judge の「現在版が prerelease なら候補にプレリリースを残す」ルールが働くと安定版 release がそのまま候補になり、移動する開発ビルドへの参照が固定 release へ黙って変わっていた。`.Final` / `-jre` / `.RELEASE` / `-SP1` のような**安定版 qualifier** は従来どおり更新対象
- Gradle の 1 行完結の `ext { x = '1.0' }` も変数として解決・書き戻しできる。以前は開始行の本文を読まずに次行へ進んでいたため変数が登録されず、それを参照する依存が「解決できない変数」として依存ごと無言で消えていた (複数行形式なら解決できるので、同じ意味の記述で挙動が割れていた)。書き戻しは変数名を照合してから置換するので、`ext { a = '1'; b = '2' }` のように 1 行へ複数並んでいても隣の変数を壊さない
- Cargo.toml の dotted key で書かれた git tag 依存 (`mylib.git = "..."` + `mylib.tag = "v1.0.0"`) も書き戻せる。toml クレートは dotted key を inline table と同じ構造へ畳むため parse が git tag 依存として surface しており、`update_git_tag` が対応していないと「更新あり」と報告した後に書き込みが失敗する (`update_version` 側の dotted key 対応と非対称だった)
- `path` / `registry` という名前のクレート (どちらも crates.io に実在) も更新できる。非 crates.io ソースの判定は**更新対象パッケージの宣言行の範囲に限って**適用し、行頭の `path = "0.1.1"` を「path 依存の宣言」と誤認しない。`foo = { path = "../x" }` / `foo = { registry = "internal" }` は従来どおり除外する
- Python は PEP 440 local version 付きの固定指定 (`==2.1.0+cu121` / `===...` / Poetry の bare ピン) を書き換え不能な制約として扱い、`SkipReason::ParseError` で skip 行に出す。local label は「同じ公開バージョンから派生した別ビルド」を指すので、公開バージョンを進めるとラベルの意味が失われる (`torch==2.1.0+cu121` を PyPI の `2.5.1` で上書きすると、そもそも存在しないビルドを指す)。比較用の `version` は `2.1.0+cu121` のまま保持するため PEP 440 順の比較と OSV 照合は従来どおり効く
- Poetry の primary source (`[[tool.poetry.source]]` の `priority = "default"` / `"primary"` / priority 欠落 / legacy `default = true`)、uv の `[[tool.uv.index]] default = true` と `[tool.uv] index-url`、PDM の `[[tool.pdm.source]]` による `pypi` 上書きがある `pyproject.toml` は、依存を丸ごと更新対象から外して警告を 1 度出す。これらは暗黙の PyPI を無効化する設定なので、依存名が同じでも別物のパッケージを指す (Cargo の `registry = "..."` 除外と同型の dependency confusion 対策)。`name = "pypi"` 併記・`supplemental` / `explicit` / legacy `secondary = true` は PyPI が生きているので対象外
- `pyproject.toml` の書き換えは**論理パス**方式で行う。セクションヘッダのキー列と行頭のドットキーを連結したパス (`project` + `optional-dependencies.dev` → `project.optional-dependencies.dev`) を作り、parse が読む論理パス集合と突き合わせる。TOML 仕様上等価な dotted key (`optional-dependencies.dev = [...]`) と inline table (`optional-dependencies = { dev = [...] }`) を parse だけが読んで writer が書けない矛盾を構造的に無くすため。パッケージ名の一致もセグメント完全一致になり、`serde` → `serde_json` 型の前方一致事故が起きない
- TOML のマルチライン文字列 (`"""` / `'''`) の検出はコメントとクォートの外側に限る。行全体をそのまま探索していたときは、コメント内 (`# """ を使わないこと`) や単一引用符文字列内の区切りを開始と誤認し、閉じる区切りが現れるまで**ファイルの残り全部が更新不能**になっていた (report は「更新あり」なので apply と矛盾する)。`pyproject.toml` と `mise.toml` が同じ問題を各自で塞いでいる (将来 `line_utils` へ共通化する余地がある)
- mise は top-level の dotted key (`tools.node = "20.0.0"` / `tools."npm:prettier" = "..."` / `tools.node.version = "..."`) も書き戻せる。`tools = { node = ... }` の inline table 形式と `node = """26.7.0"""` のマルチライン文字列値は安全に書き換えられないため parse 側でも除外して report/apply を整合させる。parse と update が同じ locator を共有するので、locator が位置を特定できない書き方は必ず両方でスキップされる
- mise の table オプションは `version` に加えて `prefix` も読む (`node = { prefix = "26" }` は文字列形式の `"prefix:26"` と同義で `VersionSpecKind::Prefix`)。`path` / `ref` のみのテーブルは文字列形式と同じく更新対象外。同義の 2 記法で挙動が割れないようにする
- mise のツール単位 `minimum_release_age` (`node = { version = "22", minimum_release_age = "30d" }`) は depup 側で解釈しないため、設定されていれば警告を出す (`[settings]` の `minimum_release_age_excludes` と同じ扱い。片方だけ無言だと非対称になる)
- `go.work` の `use` と `settings.gradle` / `settings.gradle.kts` の `include` を展開してサブプロジェクトのマニフェストも検出する。ルートに `go.mod` / 依存宣言が無い構成では展開しないと「更新なし」と報告して**無言の no-op** になる (Cargo workspace members と pnpm workspace packages には自動展開があるので Go と Java だけ非対称だった)。展開先のパスは `.depup` と同じ安全性検査 (絶対パス・`..`・外部へ解決される symlink の拒否) を通す。検出ソースが増えたため、重複排除は `push_unique_manifest` に集約している
- GitHub Tags の semver 受理判定は `SwiftVersionParser` へ委譲して `Package.swift` 側と一元化する。取得側だけが数値識別子の先頭ゼロを許していたため、`2024.01.15` のような CalVer タグが (semver タグと混在すると `2024 > 1` で必ず) 最新候補に選ばれ、SPM の `Version` が読めない `Package.swift` を書き込んでいた。書き込み側でも `new_version` の妥当性を検証する。タグが 1000 件 (10 ページ) を超えて打ち切られた場合は警告を出す (無言で切ると古い系列が候補から静かに落ちる)
- `GITHUB_TOKEN` / `GH_TOKEN` は空文字を「未設定」として読み飛ばす。`std::env::var` は「セット済みだが空文字」に対して `Ok("")` を返すため、空値をそのまま採用すると `Authorization: Bearer ` (トークン部が空) を送って GitHub に 401 で拒否され、ヘッダを付けなければ未認証で成功していた取得が全滅する。空の `GITHUB_TOKEN` が置かれた環境で `GH_TOKEN` へフォールバックできない問題も同時に防ぐ
- レジストリのパッケージ名は URL へ埋め込む前に文字種を検証する (npm / PyPI / crates.io / RubyGems / Packagist / Maven Central / GitHub Tags)。`reqwest` が使う `url` crate は WHATWG URL 仕様どおりドットセグメントを正規化するため、`"a/../lodash": "^1.0.0"` のようなキーがあると lodash の版を取得して元のキーへ書き戻す。`?` / `#` はクエリ・フラグメントとして解釈される。npm alias の実名はマニフェストの**値**から切り出されるため、名前の出所はキーだけではない
- RubyGems の `created_at` と PyPI の `upload_time_iso_8601` が欠落・不正な場合は `UNIX_EPOCH` へフォールバックする (候補ごと捨てない)。Packagist の `time` 欠損と Go proxy の `.info` の `Time` 欠落が既に同じ方針なので、レジストリ層の一貫性を揃える
- `--diff` の値には npm alias の `npm:<real>@` 接頭辞も含める。`version_spec.raw` は制約部分 (`^17.0.0`) しか持たないため、接頭辞を落とすと diff が「alias 宣言が外れて素の `^18.0.0` になる」ように見え、実書き込み (`"npm:@preact/compat@^18.0.0"`) と食い違う。接頭辞は `Dependency::value_prefix` に保持する
- テキスト / JSON 出力の「現在のバージョン」は `VersionSpec::display_version()` を使い、`v` / `V` 接頭辞を復元する。更新先はレジストリが返す生の値なので、Go では `1.3.0 → v1.6.0` という不揃いな表示になり JSON の `from` / `to` も書式が食い違っていた。`^` / `~` / `>=` は制約を表す演算子であってバージョンの一部ではないため含めない
- `--age` の transitive 対応可否の通知は**言語単位ではなく実際に選ばれる PM 単位**で判定する (`Language::native_transitive_age_package_managers`)。Node の transitive age は pnpm、Python は uv だけの機能なので、言語で判定すると npm / yarn / bun / pip / poetry / rye / pipenv のプロジェクトで通知が出ず、「transitive にも cooldown が効いた」と誤解させる
- crates.io の 1 リクエスト/秒 間隔は `CratesIoRateLimit` を `Orchestrator` が 1 つ持って全アダプタへ配り、実行全体で共有する。アダプタのインスタンスに閉じ込めるとマニフェスト境界やフェーズ境界 (check → post-install の lock 監査) で間隔がリセットされ、直前のリクエストから 1 秒経たずに次が飛ぶ
